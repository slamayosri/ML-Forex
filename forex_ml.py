...
def percentChange(startPoint, currentPoint):
	try:
	x = return ((float(currentPoint) - startPoint)/abs(startPoint))*100.00)
	if x == 0.0:
		return 0.00000001
	else:
		return x
...
plotPatAr = []
	...
	if howSim>70:
		...
		if patFound == 1:
			...
			predictedAvgOutcomes = reduce(lambda x, y: x+y, predictedOutcomesAr / len(predictedOutcomesAr))
	...
	plyt.subplots_adjust(bottom=.23)
...
while toWhat<dataLength:
	...
	print 'Entire process time took', totalTime, 'seconds'
